...
void run_queue_sync_test(bool is_event_sync_mode)
{
    ...
    const int size = 128 * 1024 * 1024; // 128MB
    ...
    std::cout << "Copy time: " << elapsed_1.count() << " ms, size = " << size / 1024 << " KB" << std::endl;
...
void run_tests_1()
{
    ...
    cl_context context_1 = clCreateContext(nullptr, 1, &device, nullptr, nullptr, nullptr);
    cl_context context_2 = clCreateContext(nullptr, 1, &device, nullptr, nullptr, nullptr);

    cl_command_queue_properties props[] = {CL_QUEUE_PROPERTIES, 0, 0};
    cl_command_queue queue_1 = clCreateCommandQueueWithProperties(context_1, device, props, nullptr);
    cl_command_queue queue_2 = clCreateCommandQueueWithProperties(context_2, device, props, nullptr);

    const int size = 128 * 1024 * 1024; // 128MB
    std::vector<uint8_t> A(size, 0x7);
    std::vector<uint8_t> result(size, 0x0);
    cl_mem bufA_0 = clCreateBuffer(context_1, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, sizeof(int8_t) * size, A.data(), nullptr);
    cl_mem bufA_1 = clCreateBuffer(context_1, CL_MEM_READ_WRITE, sizeof(int8_t) * size, nullptr, nullptr);

    cl_mem bufB_0 = clCreateBuffer(context_2, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, sizeof(int8_t) * size, A.data(), nullptr);
    cl_mem bufB_1 = clCreateBuffer(context_2, CL_MEM_READ_WRITE, sizeof(int8_t) * size, nullptr, nullptr);

    cl_event event;
    clEnqueueCopyBuffer(queue_1, bufA_0, bufA_1, 0, 0, size, 0, NULL, &event);
    clEnqueueCopyBuffer(queue_2, bufB_0, bufB_1, 0, 0, size, 1, &event, nullptr);
...
int main(int argc, char **argcv)
{
    ...
    run_queue_sync_test(0); // manual sync(clFinish) will sucess
    run_queue_sync_test(1); // event sync will fail
    return 1;
}